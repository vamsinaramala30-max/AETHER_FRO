  ...
  if (!activityData || activityData.length === 0) {
    return (
      <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-6 text-center dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="text-sm font-bold text-slate-700 dark:text-slate-300">No knowledge activity yet</p>
        ...
        {activityData.slice(-14).map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center gap-2 group relative">
              {/* Tooltip */}
              <div className="absolute -top-8 hidden group-hover:flex rounded-lg bg-slate-900 px-2 py-1 text-[10px] font-bold text-white shadow-md z-20 whitespace-nowrap dark:bg-slate-800">
                {item.count} activity event(s)
              </div>

              {/* Bar */}
              <div className="w-full max-w-[28px] bg-slate-100 dark:bg-slate-800/80 rounded-t-lg overflow-hidden flex items-end h-24">
                <div
                  className="w-full bg-gradient-to-t from-indigo-600 to-purple-500 rounded-t-lg transition-all duration-300 group-hover:from-indigo-500 group-hover:to-purple-400"
                  style={{ height: `${heightPercent}%` }}
                />
              </div>

              {/* Label */}
              <span className="text-[10px] font-bold text-slate-500 truncate max-w-[40px] text-center dark:text-slate-400">
