...
export const StudyPlannerPage: React.FC = () => {
  ...
  useEffect(() => {
    void (async () => {
      const data = await studyPlannerService.getSessions();
      setSessions(data);
      setLoading(false);
    })();
  }, []);
  ...
  const handleCreateSession = (e: React.SyntheticEvent) => {
    e.preventDefault();
    if (topic.trim() === '' || time.trim() === '') return;

    void (async () => {
      try {
        const created = await studyPlannerService.addSession({
          topic,
          moduleName: modName.trim() !== '' ? modName : 'General Sandbox',
          scheduledTime: time,
          durationMinutes: dur,
          completed: false,
        });
  ...
  if (loading)
    return (
      <div style={{ color: '#0066cc', padding: '2rem', textAlign: 'center' }}>
        Synchronizing cognitive schedule map...
      </div>
    );
    ...
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', color: '#fff' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 800 }}>
          Deep-Work Scheduling Matrix
        </h1>
        <p style={{ margin: '0.25rem 0 0 0', color: '#888' }}>
          Isolate complex paradigms into scheduled architectural sprints.
        </p>
      </div>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          >
            ...
            <input
              type="text"
              placeholder="Deep-Dive Topic Definition"
              value={topic}
              onChange={(e) => {
            ...
            <input
              type="text"
              placeholder="Context Module (e.g., Concurrency)"
              value={modName}
              onChange={(e) => {
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <input
                ...
                value={dur}
                onChange={(e) => {
                  setDur(parseInt(e.target.value, 10) || 30);
                }}
                min="15"
                style={{
                  flex: 1,
