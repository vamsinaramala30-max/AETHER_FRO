...
/* ---------------------------------------------------------------------
   TOKENS — warm paper white, ink-on-paper instrument
--------------------------------------------------------------------- */
const COLORS = {
  page: "#F5F2EA",
  panel: "#FFFFFF", panel2: "#EFEADC", line: "#DFD8C4",
  text: "#221F19", muted: "#847E6C",
  sleep: "#4C5FB8", study: "#A8631A", other: "#227A63",
  p0: "#B8402F", p1: "#A8631A", p2: "#4C5FB8", p3: "#9A9480",
  revision: "#6E52B5",
};
...
const fmtHM = (decHour: number): string => {
  let h = ((Math.floor(decHour) % 24) + 24) % 24;
...
export default function AetherWeeklyPlanner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await window.storage?.set(STORAGE_KEY, JSON.stringify({ daysData })); }
