...
function WeatherIcon({ code, isDay, size = 26 }: { code: number; isDay: boolean; size?: number }) {
  const Icon = WEATHER_ICON_COMPONENTS[getWeatherIconKey(code, isDay)];
  return (
    <span
      role="img"
      aria-label={getWeatherAriaLabel(code, isDay)}
    ...
    <div className="weather-hero">
      {isLoading || !current ? (
        ...
        <>
          ...
          <div className="weather-hero__condition">{condition?.label ?? 'Unavailable'}</div>
          <div className="weather-hero__range">
            {location ? (
              <>
                Feels like{' '}
                {formatTemperature(current.apparentTemperature, unit, { withUnit: true })}
              </>
            ) : (
              'Unavailable'
            )}
          ...
          : daily.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="daily-row__label">
                    {getRelativeDayLabel(day.date, timezone)}
                  </span>
                  <WeatherIcon code={day.weatherCode} isDay size={22} />
                  <span className="daily-row__min">
                    {formatTemperature(day.temperatureMin, unit)}°
                  </span>
                  <span className="daily-row__max">
  ...
  const items: Array<{ label: string; value: string; icon: LucideIcon }> = current
    ? [
        ...
        },
        {
          ...
          value:
            current.windSpeedKmh !== null ? getBeaufortForce(current.windSpeedKmh) : 'Unavailable',
          icon: Wind,
        },
...
function AirQualitySection({
  ...
  const pollutants: Array<{ label: string; value: number | null; unit: string; max: number }> =
    airQuality
      ? [
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div
              ...
              role="img"
              aria-label={`Air quality index ${Math.round(aqiValue)}, ${category.label}`}
            >
              {barPosition !== null && (
      ...
      <div className="weather-screen">
        <div className="weather-topbar">
          ...
          <div className="weather-topbar__actions">
            <IconButton
              label={editMode ? 'Done editing' : 'Edit locations'}
        ...
        {query.trim().length >= 2 && (
          <div className="city-mgmt__results" role="listbox" aria-label="City search results">
            ...
            {status === 'error' && (
              <div style={{ padding: 16, color: 'rgba(20,23,43,0.6)' }}>
        ...
        {savedLocations.length === 0 ? (
          <p className="city-mgmt__empty">
            No saved cities yet. Search above to add one, or allow location access to add your
            current city.
          </p>
        ) : (
...
export default function Weather() {
  ...
  const loadWeatherFor = useCallback(async (location: WeatherLocation, skipCache = false) => {
    ...
    if (result.ok) {
      ...
      setSelectedDate(
        result.data.daily[0]?.date ?? getTodayInTimezone(result.data.location.timezone),
      );
  ...
  const requestDeviceLocation = useCallback(async () => {
    ...
    if (!position.ok) {
      const stateMap: Record<
        string,
        { permission: LocationPermissionState; weather: WeatherState }
      > = {
  ...
  const selectedDayHourly = effectiveSelectedDate
    ? (hourlyByDate.get(effectiveSelectedDate) ?? [])
    : [];
  const selectedDayLabel =
    effectiveSelectedDate && activeLocation
          ...
          <>
            ...
            {weatherData && !isLoading && (
              ...
              >
                ...
                {' ~ '}
                {formatTemperature(weatherData.daily[0]?.temperatureMax ?? null, unit, {
                  withUnit: true,
                })}
              </p>
            )}
            ...
            {!isLoading && selectedDayHourly.length > 1 && (
              <section
                ...
                aria-label={`Temperature trend for ${selectedDayLabel}`}
              >
