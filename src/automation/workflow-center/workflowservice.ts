...
let mockWorkflows: Workflow[] = [
  {
    ...
    nodes: [
      { id: 'node-1', type: 'trigger', name: 'Webhook Received', config: { path: '/v1/deploy-status' } },
      { id: 'node-2', type: 'condition', name: 'If Status == "failed"', config: { field: 'status', operand: 'failed' } },
      { id: 'node-3', type: 'action', name: 'Send Slack Notification', config: { channel: '#ops-alerts' } }
    ]
  },
  {
    id: 'wf-2',
    name: 'AI Lead Qualification sync',
    description: 'Parses inbound contact forms via AETHER AI core and syncs qualified profiles to internal storage.',
    isActive: false,
    triggerType: 'CRM Event',
    lastTriggeredAt: '2026-07-18T09:11:00Z',
    createdAt: '2026-03-22T11:45:00Z',
    nodes: [
      { id: 'node-a', type: 'trigger', name: 'New Contact Form Submited', config: { source: 'website' } },
      { id: 'node-b', type: 'action', name: 'AETHER AI Enrichment', config: { model: 'aether-fast-v2' } },
      { id: 'node-c', type: 'action', name: 'Update Data Registry', config: { collection: 'leads' } }
    ]
  }
];
...
export const workflowService = {
  async getWorkflows(): Promise<Workflow[]> {
    return new Promise((resolve) => {
      setTimeout(() => { resolve([...mockWorkflows]); }, 350);
    });
  },
  ...
  async toggleWorkflow(id: string): Promise<Workflow> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (index === -1) { reject(new Error('Workflow not found')); return; }
        mockWorkflows[index] = {
          ...mockWorkflows[index],
