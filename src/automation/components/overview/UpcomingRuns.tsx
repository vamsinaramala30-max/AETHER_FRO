    ...
    <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Next scheduled background executions</p>
        </div>
      ...
      {scheduledList.length === 0 ? (
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {scheduledList.slice(0, 5).map((item) => (
            <div key={item.id} className="flex items-center justify-between py-3 first:pt-4 last:pb-0">
